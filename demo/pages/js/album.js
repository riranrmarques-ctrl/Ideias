...
async function loadAlbum() {
  const params = new URLSearchParams(window.__demoSearch());
  const albumId = params.get('id');
  if (!albumId) { window.location.href = 'portfolio.html'; return; }

  const user = await trySession();
  el('backLink').href = user ? 'minha-area.html' : 'portfolio.html';
  ...
  if (!res.ok) { window.location.href = 'portfolio.html'; return; }
  ...
  btn.addEventListener('click', async () => {
    ...
    if (panel.style.display === 'none') {
      ...
      document.getElementById('genAlbumLinkBtn').addEventListener('click', async () => {
        ...
        if (res.ok) {
          const url = `${window.__demoBase()}compartilhado.html?t=${data.token}`;
      ...
      document.getElementById('guestUploadToggle').addEventListener('change', async (e) => {
        ...
        if (data.token) {
          const url = `${window.__demoBase()}convidados.html?t=${data.token}`;
          box.style.display = 'block';
          box.innerHTML = `${url} <button class="icon-btn" style="color:var(--gold);" onclick="navigator.clipboard.writeText('${url}'); this.textContent='copiado!'">copiar</button>`;
        } else {
